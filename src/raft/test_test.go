...
func TestReElection2A(t *testing.T) {
	...
	log.Printf("first check--------------------------leader：%v", leader1)
	...
	time.Sleep(2 * RaftElectionTimeout)
	log.Printf("third check--------------------------")
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	DPrintf("four connect: %v", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	log.Printf("four check--------------------------")
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	log.Printf("five check--------------------------")
	cfg.checkOneLeader()
...
func TestFailNoAgree2B(t *testing.T) {
	...
	DPrintf("TestFailNoAgree2B, leader1: %v", leader)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("TestFailNoAgree2B, leader2: %v", leader2)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("TestRejoin2B, leader1: %v", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf("TestRejoin2B, leader2: %v", leader2)
	// old leader connected again
	cfg.connect(leader1)

	cfg.one(104, 2, true)
	DPrintf("TestRejoin2B, 3")
	// all together now
	cfg.connect(leader2)
